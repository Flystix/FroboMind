...
ros::Publisher  accPublisher;		// v adxl345 		-> fmMsgs::accelerometer /accData
ros::Publisher  gyroPublisher;		// v itg3200 		-> fmMsgs::gyroscope /gyroData
ros::Publisher  magPublisher;		// v hmc5883 		-> fmMsgs::magnetometer /magData
ros::Publisher  radioPublisher;		// v avr     		-> fmMsgs::airframe_control /radioData
ros::Publisher  batteryPublisher;	// v avr  			-> fmMsgs::battery /batteryData
ros::Publisher  pitotPublisher;		// v avr     		-> fmMsgs::airSpeed /pitotData
ros::Publisher  rangePublisher;		// v avr     		-> fmMsgs::altitude /altData
...
void avrSetControlsCallback(const fmMsgs::airframeControl::ConstPtr& msg) {
	...
	avr_ptr->set_servos(&servos);
}

void avrDataCallback(float (*radio)[5], int manOverrule, float vAir, float range, float battery, ros::Time timestamp) {
	...
	vAir_lp = 0.99 * vAir_lp + 0.01 * vAir;
