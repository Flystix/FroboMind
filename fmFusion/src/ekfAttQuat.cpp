...
#define DEBUG_STR(str)	ROS_INFO(str); ROS_INFO("[%2.2f , %2.2f , %2.2f]",x(1), x(2), x(3));
#else
...
ekfAttQuat::ekfAttQuat(double processVariance, double measurementVariance) {
	//	setSizeX(2);	//!< Size of the state vector. [q0 q1 q2 q3 bgx bgy bgz]
	//	setSizeU(3);	//!< Size of the input vector. [gx gy gz]
	//	setSizeW(2);	//!< Size of the process noise vector. (1x4?) [wgx wgy wgz]
	//	setSizeZ(3);	//!< Size of the measurement vector. [ax ay az]
	//	setSizeV(3); 	//!< Size of the measurement noise vector. [vax vay vaz]
	setDim(4, 3, 3, 3, 3);
	xEuler.resize(2);
...
//	if (isnan(x(1)) || isnan(x(2)) || isnan(x(3)) || isnan(x(4))) {
//		x(1) = 1.0;
//		x(2) = 0.0;
//		x(3) = 0.0;
//		x(4) = 0.0;
...
void ekfAttQuat::updatedt(double newdt) {
	dt = newdt;
}

const ekfAttQuat::Vector& ekfAttQuat::getXEuler(void) {
...
void ekfAttQuat::reset(double initPhi, double initTheta) {
	double ph = initPhi / 2;
