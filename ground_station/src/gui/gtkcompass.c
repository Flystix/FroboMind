 ...
 */
enum _GTK_COMPASS_PROPERTY_ID {
	PROP_0, PROP_GRAYSCALE_COLOR, PROP_RADIAL_COLOR,
...
static void gtk_compass_set_property(GObject * object, guint prop_id, const GValue * value,
                                     GParamSpec * pspec);
...
static void gtk_compass_class_init(GtkCompassClass * klass) {
	...
	g_object_class_install_property(obj_class, PROP_GRAYSCALE_COLOR,
	...
	g_object_class_install_property(obj_class, PROP_RADIAL_COLOR,
	                                g_param_spec_boolean("radial-color",
	                                                     ...
	                                                     "the widget use radial color", TRUE,
	                                                     G_PARAM_WRITABLE));
	return;
}
...
static void gtk_compass_init(GtkCompass * comp) {
	...
	gtk_widget_add_events(GTK_WIDGET (comp), GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK
...
static gboolean gtk_compass_expose(GtkWidget * comp, GdkEventExpose * event) {
	...
	if (!priv->draw_once) {
		priv->static_surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, event->area.width,
		                                                  event->area.height);
		cr_static = cairo_create(priv->static_surface);
		cairo_rectangle(cr_static, event->area.x, event->area.y, event->area.width,
		                event->area.height);
	...
	priv->dynamic_surface = cairo_surface_create_similar(priv->static_surface,
...
static void gtk_compass_draw_base(GtkWidget * comp, cairo_t * cr) {
	...
	cairo_arc(cr, rec_x0 + rec_width - rec_radius, rec_y0 + rec_radius, rec_radius, -90
	        * rec_degrees, 0 * rec_degrees);
	cairo_arc(cr, rec_x0 + rec_width - rec_radius, rec_y0 + rec_height - rec_radius, rec_radius, 0
	        * rec_degrees, 90 * rec_degrees);
	cairo_arc(cr, rec_x0 + rec_radius, rec_y0 + rec_height - rec_radius, rec_radius, 90
	        * rec_degrees, 180 * rec_degrees);
	cairo_arc(cr, rec_x0 + rec_radius, rec_y0 + rec_radius, rec_radius, 180 * rec_degrees, 270
	        * rec_degrees);
	cairo_close_path(cr);

	if (((priv->radial_color) && (priv->grayscale_color)) || ((!priv->radial_color)
	        && (priv->grayscale_color)) || ((!priv->radial_color) && (!priv->grayscale_color))) {
		if (!priv->grayscale_color)
			...
			cairo_set_source_rgb(cr, (gdouble) priv->bg_color_inv.red / 65535,
			                     (gdouble) priv->bg_color_inv.green / 65535,
			                     (gdouble) priv->bg_color_inv.blue / 65535);
	} else {
		pat = cairo_pattern_create_radial(x - 0.392 * radius, y - 0.967 * radius, 0.167 * radius, x
		        - 0.477 * radius, y - 0.967 * radius, 0.836 * radius);
		cairo_pattern_add_color_stop_rgba(pat, 0,
		                                  (gdouble) priv->bg_radial_color_begin_bounderie.red
		                                          / 65535,
		                                  (gdouble) priv->bg_radial_color_begin_bounderie.green
		                                          / 65535,
		                                  (gdouble) priv->bg_radial_color_begin_bounderie.blue
		                                          / 65535, 1);
	...
	if (((priv->radial_color) && (priv->grayscale_color)) || ((!priv->radial_color)
		...
		pat = cairo_pattern_create_radial(x - 0.392 * radius, y - 0.967 * radius, 0.167 * radius, x
		        - 0.477 * radius, y - 0.967 * radius, 0.836 * radius);
		cairo_pattern_add_color_stop_rgba(pat, 0, (gdouble) priv->bg_radial_color_begin_compass.red
		        / 65535, (gdouble) priv->bg_radial_color_begin_compass.green / 65535,
		                                  (gdouble) priv->bg_radial_color_begin_compass.blue
		                                          / 65535, 1);
...
static void gtk_compass_draw_screws(GtkWidget * comp, cairo_t * cr) {
	...
	pat = cairo_pattern_create_radial(x - 0.82 * radius, y - 0.82 * radius, 0.07 * radius, x - 0.82
	        * radius, y - 0.82 * radius, 0.1 * radius);
...
static void gtk_compass_draw_course(GtkWidget * comp, cairo_t * cr) {
	...
	cairo_move_to(cr,
	              ...
	              y + (radius - 0.0 * radius) * sin(-3 * M_PI / 6));
	cairo_line_to(cr,
	              x + (radius - 0.2 * radius) * cos(M_PI / 40 - 3 * M_PI / 6),
	              y + (radius - 0.2 * radius) * sin(M_PI / 40 - 3 * M_PI / 6));
	cairo_line_to(cr,
	              x + (radius - 0.2 * radius) * cos(-M_PI / 40 - 3 * M_PI / 6),
	              y + (radius - 0.2 * radius) * sin(-M_PI / 40 - 3 * M_PI / 6));
	cairo_move_to(cr,
...
static void gtk_compass_draw_tips_and_numbers(GtkWidget * comp, cairo_t * cr) {
	...
	for (i = 0; i < 12; i++) {
		...
		switch (i) {
			case 0:
				// N
				cairo_move_to(cr, x + (radius - inset) * cos(i * M_PI / 6 - M_PI / 32 - 3 * M_PI
				        / 6), y + (radius - inset) * sin(i * M_PI / 6 - M_PI / 32 - 3 * M_PI / 6));
				break;
			case 3:
				// E
				cairo_move_to(cr, x + (radius - inset) * cos(i * M_PI / 6 - M_PI / 34 - 3 * M_PI
				        / 6), y + (radius - inset) * sin(i * M_PI / 6 - M_PI / 34 - 3 * M_PI / 6));
				break;
			case 6:
				// S
				cairo_move_to(cr, x + (radius - inset) * cos(i * M_PI / 6 - M_PI / 35 - 3 * M_PI
				        / 6), y + (radius - inset) * sin(i * M_PI / 6 - M_PI / 35 - 3 * M_PI / 6));
				break;
			case 9:
				// W
				cairo_move_to(cr, x + (radius - inset) * cos(i * M_PI / 6 - M_PI / 24 - 3 * M_PI
				        / 6), y + (radius - inset) * sin(i * M_PI / 6 - M_PI / 24 - 3 * M_PI / 6));
				break;
			default:
				if (i * 3 / 10 % 10 == 0) {
					cairo_move_to(cr, x + (radius - inset) * cos(i * M_PI / 6 - M_PI / 34 - 3
					        * M_PI / 6), y + (radius - inset) * sin(i * M_PI / 6 - M_PI / 34 - 3
					        * M_PI / 6));
				} else {
					cairo_move_to(cr, x + (radius - inset) * cos(i * M_PI / 6 - M_PI / 18 - 3
					        * M_PI / 6), y + (radius - inset) * sin(i * M_PI / 6 - M_PI / 18 - 3
					        * M_PI / 6));
				}
		}
	...
	for (i = 0; i < 36; i++) {
		...
		cairo_move_to(cr, x + (radius - inset) * cos(i * M_PI / 18), y + (radius - inset) * sin(i
		        * M_PI / 18));
		cairo_line_to(cr, x + radius * cos(i * M_PI / 18), y + radius * sin(i * M_PI / 18));
		cairo_stroke(cr);
		cairo_restore(cr);
	}
	cairo_restore(cr);
