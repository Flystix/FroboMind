...
extern "C" G_MODULE_EXPORT gboolean on_mainwindow_delete_event(GtkWidget *window, GdkEvent *event,
                                                               AppData * data) {
	if (data->recording) {
...
extern "C" G_MODULE_EXPORT gboolean on_mainwindow_key_press_event(GtkWidget * widget,
                                                                  GdkEventKey * event,
                                                                  AppData * data) {
		...
		switch (event->keyval) {
			...
			case GDK_Left:
				...
				if (data->current_page > 0)
					gtk_notebook_set_current_page(GTK_NOTEBOOK (data->notebook), data->current_page
					        - 1);
				break;
			case GDK_Right:
				// **** Switch page
...
extern "C" G_MODULE_EXPORT void on_notebook1_switch_page(GtkNotebook * notebook,
	...
	switch (page_num) {
		case 2:
			...
			while (fgets(topics_list, sizeof(topics_list), topics)) {
				...
				while (valid) {
					gtk_tree_model_get_value(GTK_TREE_MODEL (data->topicsList), &iter, 0,
					                         &topic_name_present);
					add_to_list = TRUE;
					if (strcmp(g_value_get_string(&topic_name),
					           g_value_get_string(&topic_name_present)) == 0) {
						add_to_list = FALSE;
				...
				if (add_to_list) {
					...
					gtk_list_store_set_value(GTK_LIST_STORE (data->topicsList), &iter, 0,
					                         &topic_name);
					gtk_list_store_set_value(GTK_LIST_STORE (data->topicsList), &iter, 1,
					                         &topic_selected);
				}
...
extern "C" G_MODULE_EXPORT void on_colorbutton_BgColor_color_set(GtkColorButton * button,
                                                                 AppData * data) {
	GdkColor bg_color;
...
extern "C" G_MODULE_EXPORT void on_button_OpenTelemetryOptionPopup_clicked(GtkButton * button,
                                                                           AppData * data) {
	data->telemetry_opt_popup_state = true;
	gtk_widget_hide_all(data->btn_open_telemetry_option_popup);
	gtk_widget_show_all(data->telemetry_option_popup);
...
extern "C" G_MODULE_EXPORT void on_button_CloseTelemetryOptionPopup_clicked(GtkButton * button,
                                                                            AppData * data) {
	data->telemetry_opt_popup_state = false;
	gtk_widget_hide_all(data->telemetry_option_popup);
	gtk_widget_show_all(data->btn_open_telemetry_option_popup);
...
extern "C" G_MODULE_EXPORT void on_checkbuttonDrawCurrentUAVTrack_toggled(
...
extern "C" G_MODULE_EXPORT void on_checkbuttonLockViewUAV_toggled(GtkToggleButton * togglebutton,
                                                                  AppData * data) {
	if (data->lock_view)
...
extern "C" G_MODULE_EXPORT void on_button_OpenGpsdOptionPopup_clicked(GtkButton * button,
                                                                      AppData * data) {
	data->gps_opt_popup_state = true;
	gtk_widget_hide_all(data->btn_open_gpsd_option_popup);
	gtk_widget_show_all(data->gpsd_option_popup);
...
extern "C" G_MODULE_EXPORT void on_button_CloseGpsdOptionPopup_clicked(GtkButton * button,
                                                                       AppData * data) {
	data->gps_opt_popup_state = false;
	gtk_widget_hide_all(data->gpsd_option_popup);
	gtk_widget_show_all(data->btn_open_gpsd_option_popup);
...
extern "C" G_MODULE_EXPORT void on_combobox_MapProvider_changed(GtkComboBox * box, AppData * data) {
	...
	osm_gps_map_convert_screen_to_geographic(data->map,
	                                         (gint) (data->map_container)->allocation.width / 2,
	                                         (gint) (data->map_container)->allocation.height / 2,
	                                         &current_coord);
	...
	osm_gps_map_set_center_and_zoom(data->map, (current_coord.rlat / M_PI * 180.0),
	                                (current_coord.rlon / M_PI * 180.0), data->map_current_zoom);
...
extern "C" G_MODULE_EXPORT void on_treeview2_topics_row_activated(GtkTreeView * test,
                                                                  GtkTreePath * path,
	...
	if (strcmp(data->file_prefix, "") != 0) {
		sprintf(data->cmd_line, "%s -o %s %s", data->rosbag_record_cmd, data->file_prefix,
		        data->list_topic);
...
extern "C" G_MODULE_EXPORT void on_button_UpdateTopicList_clicked(GtkButton * button,
                                                                  AppData * data) {
	GtkTreeIter iter;
	GValue topic_name = { 0, }, topic_selected = { 0, };
	GValue topic_name_present = { 0 };
	...
	while (fgets(topics_list, sizeof(topics_list), topics)) {
		...
		while (valid) {
			gtk_tree_model_get_value(GTK_TREE_MODEL (data->topicsList), &iter, 0,
			                         &topic_name_present);
			add_to_list = TRUE;
			if (strcmp(g_value_get_string(&topic_name), g_value_get_string(&topic_name_present))
			        == 0) {
				add_to_list = FALSE;
				g_value_unset(&topic_name_present);
				break;
